...
namespace fingerPressure
{
    public partial class UCheckComboBox : ComboBox
    {
        ...
        public void BindingDataList(List<string> data)
        {
            valueProperty = null;
            ...
            // 设置下拉框显示高度
            int sheight = Screen.GetBounds(this).Height;
            this.panel.Size = new Size(this.panel.Width, Math.Min(data.Count * 18 + 31, sheight - 60));
        }
            ...
            CheckedListBox.ItemCheck += delegate (object sender, ItemCheckEventArgs e)
            {
                ...
                for (int i = 0; i < CheckedListBox.Items.Count; i++)
                {
                    ...
                    if ((i == e.Index) != CheckedListBox.GetItemChecked(i))
                    {
                        ...
                        text += CheckedListBox.Items[i].GetType().GetProperty(displayProperty).GetValue(CheckedListBox.Items[i]).ToString() + ",";
                    }
                }
